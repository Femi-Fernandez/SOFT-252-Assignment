...
public class UserFactory {
    ...
    char firstChar = userID.charAt(0);
        ...
        if (firstChar == 'A') {
            Admin newAdmin = new Admin(userID, firstname,  surname, address);
            SystemDatabase.userArray.add(newAdmin);
        }
        if (firstChar == 'D') {
            Doctor newDoctor = new Doctor(userID, firstname,  surname, address);
            SystemDatabase.userArray.add(newDoctor);
        }
        if (firstChar == 'P') {
            Patient newPatient = new Patient(userID, firstname,  surname, address, age, gender);
            SystemDatabase.userArray.add(newPatient);
        }
        if (firstChar == 'S') {
            Secretary newSecretary = new Secretary(userID, firstname,  surname, address);
